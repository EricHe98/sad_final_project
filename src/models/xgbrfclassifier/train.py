...
from xgboost import XGBRanker, XGBClassifier, XGBRFClassifier
...
def __main__():
    ...
    with mlflow.start_run(run_name='xgbrfclassifier'):
        run_id = mlflow.active_run().info.run_id
        print('MLFlow Run ID is: {}'.format(run_id))
        mlflow.log_param('dataset', args.dataset)
        mlflow.log_param('train_split', args.split)
        mlflow.log_param('model_name', 'XGBRFClassifier')
        ...
        model = XGBRFClassifier(verbosity=2, n_jobs=-1, n_estimators=100)
